...
- **MCP Servers**
  ...
  - [kagi-search](https://github.com/web-agent-master/kagi-search) - Run Kagi web searches: query with operators, return structured results (title/link/snippet), and gather multiple sources for verification.
  - [mintlify](https://mintlify.com/docs/mcp) - Fetch documentation content from Mintlify via a remote integration (disabled by default).
  - [motion](https://api.motion.dev/registry.tgz?package=motion-studio-mcp&version=latest) - Generate CSS easing helpers: create spring-like and bounce-like linear() transitions tuned by duration/bounce for UI animation.
  ...
  - [ssh-ec2-tailscale](https://github.com/tufantunc/ssh-mcp) - Run remote shell commands over SSH to an EC2 host reachable via Tailscale: exec commands, use sudo, and fetch diagnostics for ops workflows.
  - [ssh-local-dev](https://github.com/tufantunc/ssh-mcp) - Run remote shell commands over SSH to a local dev host: exec commands, use sudo, and fetch diagnostics for development workflows.
  - [ssh-nas](https://github.com/tufantunc/ssh-mcp) - Run remote shell commands over SSH on a NAS host: exec commands, use sudo, and fetch diagnostics for ops workflows.
  - [ssh-oracle](https://github.com/tufantunc/ssh-mcp) - Run remote shell commands over SSH on an Oracle host: exec commands, use sudo, and fetch diagnostics for ops workflows.
  - [ssh-usa](https://github.com/tufantunc/ssh-mcp) - Run remote shell commands over SSH on a USA host: exec commands, use sudo, and fetch diagnostics for ops workflows.
  - [stitch](https://stitch.googleapis.com/mcp) - Generate and edit UI screens: create screens from text prompts, edit existing screens, and generate design variants (layout/color/typography/content).
  - [supabase](https://mcp.supabase.com/mcp?project_ref=jigtlcgecidrctqelsxa&features=storage%2Cbranching%2Cfunctions%2Cdevelopment%2Cdebugging%2Cdatabase%2Caccount%2Cdocs) - Work with Supabase/Postgres: manage projects and run database-related operations.
  - [vercel](https://mcp.vercel.com) - Manage Vercel projects and deployments: list teams/projects/deployments, fetch build/runtime logs, create share links for protected previews, and deploy from this repo.
