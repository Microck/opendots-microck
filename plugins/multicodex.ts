...
function isRetryStatusEvent(event: unknown): event is RetryStatusEvent {
  ...
  // Handle both "retry" and "busy" status types
  const isValidType = status.type === "retry" || status.type === "busy";
  return isValidType && typeof status.message === "string";
}
...
function isRateLimitText(message: string) {
  ...
  return (
    ...
    haystack.includes("free usage exceeded") ||
    haystack.includes("usage exceeded")
  );
}
